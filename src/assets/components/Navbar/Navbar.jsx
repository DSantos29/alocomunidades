...
function Navbar() {
  ...
  return (
    <header>
      <Link to="/" className="linkSemSublinhado">
        <h3 className="fonteLogo">ALÔ COMUNIDADES</h3>
      </Link>
      <nav ref={navRef} className="navPrincipal">
        <ul className="navUl">
          ...
          {logado ? (
            ...
            <Link to="/login">
              <li className="logado">Login</li>
            </Link>
          )}

          {logado ? (
            <Link to="/login" target="blank">
              <li className="logado" onClick={handleDeslogar}>
                Sair{" "}
              </li>
            </Link>
          ) : (
            <Link to="/Cadastro">
              <li className="logado">Cadastre-se </li>
            </Link>
          )}
        </ul>
